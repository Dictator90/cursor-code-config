"""
KV-cache hit rate analyzer for Cursor sessions.

Parses ~/.cursor/projects/*/<session-id>.jsonl files and computes, for each
session and overall:
...
- Dollar estimate using current Cursor pricing for Opus 4.6
- Distribution across sessions (worst, median, best hit rates)
- Sessions newer than N days only (default 7)

Run:
    python ~/.cursor/scripts/kvcache_stats.py
    python ~/.cursor/scripts/kvcache_stats.py --days 30 --project CODE-Cursor
    python ~/.cursor/scripts/kvcache_stats.py --top 10
"""
...
# Cursor Opus 4.6 pricing (per 1M tokens) as of April 2026
